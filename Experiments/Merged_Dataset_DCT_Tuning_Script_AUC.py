...
def dct_full_batch(batch):
    # batch: (B, H, W, C), uint8 → float32
    batch = tf.image.convert_image_dtype(batch, tf.float32)

    # 1D DCT along width (axis=2)
    x = tf.signal.dct(batch, type=2, norm='ortho')  # (B, H, W, C)

    # 1D DCT along height (axis=1) via transpose
    x = tf.signal.dct(tf.transpose(x, perm=[0, 2, 1, 3]), type=2, norm='ortho')
    x = tf.transpose(x, perm=[0, 2, 1, 3])  # back to (B, H, W, C)

    # Magnitude + grayscale
    x = tf.reduce_mean(tf.abs(x), axis=-1, keepdims=True)  # (B, H, W, 1)
...
results_file = "/home/justincb/models/hparam_results_merged_dct.csv"
results = []
