...
def dct_matrix(N=8):
    ...
    mat = tf.cos((tf.constant(tf.pi) / N) * (n + 0.5) * k)
    mat = mat * tf.sqrt(2.0 / N)
    mat = tf.tensor_scatter_nd_update(mat, [[0]], [tf.sqrt(1.0 / N)])
    return mat
...
train_dct = train_dct.cache().prefetch(tf.data.AUTOTUNE)
val_dct   = val_dct.cache().prefetch(tf.data.AUTOTUNE)
test_dct  = test_dct.cache().prefetch(tf.data.AUTOTUNE)
...
param_grid = [ # Previous test showed that DCT performs best with lower learning rates and dropout rates, so I'm expanding there
    # Light
    ...
    # Medium (baseline)
    {"lr": 1e-4, "dropout": 0.2, "conv_filters": [32, 64, 128, 256]},
    ...
    {"lr": 1e-4, "dropout": 0.3, "conv_filters": [64, 128, 256, 512]},
...
results_file = "/home/justincb/models/hparam_results_merged_dct_refined.csv"
results = []

best_auc = -1
best_model_path = "/home/justincb/models/best_dct_model_merged.h5"
