...
train_fft = train_fft.cache().prefetch(tf.data.AUTOTUNE)
...
# Function to build the FFT model
def build_fft_cnn(input_shape=(256, 256, 1), conv_filters=[32, 64, 128, 256], dense_units=128, dropout_rate=0.3):
...
param_grid = [ # Original test was uninformative, expand everywhere
    # Light
    {"lr": 1e-4, "dropout": 0.1, "conv_filters": [16, 32, 64, 128]},
    ...
    {"lr": 1e-4, "dropout": 0.4, "conv_filters": [16, 32, 64, 128]},

    # Medium (baseline)
    {"lr": 5e-5, "dropout": 0.2, "conv_filters": [32, 64, 128, 256]},
    {"lr": 5e-5, "dropout": 0.3, "conv_filters": [32, 64, 128, 256]},
    ...
    # Heavy
    {"lr": 1e-4, "dropout": 0.2, "conv_filters": [64, 128, 256, 512]},
    {"lr": 5e-5, "dropout": 0.3, "conv_filters": [64, 128, 256, 512]},
    ...
    # Mixture
    {"lr": 1e-4, "dropout": 0.2, "conv_filters": [32, 64, 64, 128]},
    {"lr": 5e-5, "dropout": 0.3, "conv_filters": [32, 64, 64, 128]},
    {"lr": 1e-4, "dropout": 0.3, "conv_filters": [32, 64, 64, 128]},
    {"lr": 1e-4, "dropout": 0.3, "conv_filters": [64, 128, 128, 256]},
    {"lr": 1e-4, "dropout": 0.4, "conv_filters": [32, 64, 64, 128]},
    {"lr": 1e-4, "dropout": 0.5, "conv_filters": [32, 64, 128, 256]},
]

results_file = "/home/justincb/models/hparam_results_merged_fft_refined.csv"
results = []

best_auc = -1
best_model_path = "/home/justincb/models/best_fft_model_merged.h5"
...
for i, params in enumerate(param_grid):
    ...
    loss, acc, auc = model.evaluate(test_fft, verbose=0)
