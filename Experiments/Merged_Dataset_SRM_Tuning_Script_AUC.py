...
def srm_residual_batch(batch):
    ...
    # Reshape to (3, 3, 1, 1):
    # 3 filters, 3×3 spatial size, 1 input channel, 1 output channel
    srm_kernels = tf.reshape(srm_kernels, (3, 3, 1, 1))
...
train_srm = train_srm.cache().prefetch(tf.data.AUTOTUNE)
val_srm   = val_srm.cache().prefetch(tf.data.AUTOTUNE)
test_srm  = test_srm.cache().prefetch(tf.data.AUTOTUNE)
...
results_file = "/home/justincb/models/hparam_results_merged_srm.csv"
results = []
