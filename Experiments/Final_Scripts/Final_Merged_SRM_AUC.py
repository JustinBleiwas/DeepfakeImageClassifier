...
param_grid = [
    ...
    # As all three of these models have the same learning rate, I'm only going to vary the dropout rate and convolutional filters

    # Group 1: Refined Grid Best AUC Space
    {"lr": 0.0005, "dropout": 0.3, "conv_filters": [32, 64, 128, 256]}, # Refined Grid best AUC
    {"lr": 0.0005, "dropout": 0.25, "conv_filters": [32, 64, 128, 256]},
    {"lr": 0.0005, "dropout": 0.35, "conv_filters": [32, 64, 128, 256]},

    # Group 2: Refined Grid Second Best AUC Space (Lower Dropout already covered)
    {"lr": 0.0005, "dropout": 0.4, "conv_filters": [32, 64, 128, 256]}, # Refined Grid second best AUC
    {"lr": 0.0005, "dropout": 0.45, "conv_filters": [32, 64, 128, 256]},

    # Group 3: Base Grid Best AUC Space
    {"lr": 0.0005, "dropout": 0.4, "conv_filters": [64, 128, 256, 512]}, # Base Grid best AUC
    {"lr": 0.0005, "dropout": 0.35, "conv_filters": [64, 128, 256, 512]},
    {"lr": 0.0005, "dropout": 0.45, "conv_filters": [64, 128, 256, 512]},

    # Group 4: Between the two Architectures
    {"lr": 0.0005, "dropout": 0.3, "conv_filters": [48, 96, 192, 384]},
    {"lr": 0.0005, "dropout": 0.35, "conv_filters": [48, 96, 192, 384]},
    {"lr": 0.0005, "dropout": 0.4, "conv_filters": [48, 96, 192, 384]},
]

results_file = "/home/justincb/models/final_results_merged_srm_AUC.csv"
results = []
